...
"switch")
	...
	;;
"mac")
	mkdir build && cd build && cmake -DAV1_VIDEO_SUPPORT=ON -DENABLE_MULTIPLAYER=ON -DCMAKE_BUILD_TYPE=Release -DCMAKE_OSX_ARCHITECTURES="x86_64;arm64" $EXTRA_CMAKE_ARGS ..
	;;
"flatpak")
	;;
"appimage")
	mkdir build && cd build && cmake -DAV1_VIDEO_SUPPORT=ON -DENABLE_MULTIPLAYER=ON -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/usr $EXTRA_CMAKE_ARGS ..
	;;
"linux")
	mkdir build && cd build && cmake -DAV1_VIDEO_SUPPORT=ON -DENABLE_MULTIPLAYER=ON -DCMAKE_BUILD_TYPE=Release $EXTRA_CMAKE_ARGS ..
	;;
"emscripten")
	export EMSDK=${PWD}/emsdk
	mkdir build && cd build && cmake -DCMAKE_BUILD_TYPE=Release -DTARGET_PLATFORM=emscripten $EXTRA_CMAKE_ARGS ..
	;;
*)
